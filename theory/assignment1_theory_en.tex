...
% Write your answer here
\subsubsection*{1}
\bold{a} If you apply a filter with a receptive field bigger than 1, and without
% TODO
\bold{insert padding name here} padding, the feature map size decrease.
\bold{b} It reduces the size of the feature map
\bold{c}
% TODO: Better formulation
Two layers are sparsely connected if the matrix $A$ in the  is mostly composed of
zeroes
\bold{d} The bias are not useful in this case, as they are eliminated by the
BatchNorm and added back by the BatchNorm layer.
\subsubsection*{2}
\bold{a} \bold{aka le nombre de channels est important ? }
\bold{b} False
\bold{c} True
\bold{d}
}
